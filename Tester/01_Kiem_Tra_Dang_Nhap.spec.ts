...
test.describe('TEST 01: DANG NHAP VA DANG KY', () => {
    ...
    test('TC03: Dang nhap that bai - sai mat khau', async ({ page }) => {
        ...
        await page.locator('#btnLogin').click();
        await expect(page.locator('#lblMessage')).toBeVisible({ timeout: 5000 });
    });
    ...
    test('TC07: Chuyen sang form dang ky', async ({ page }) => {
        await page.locator('text=Đăng ký mới').first().click();
        await expect(page.locator('text=ĐĂNG KÝ').first()).toBeVisible();
    ...
    test('TC08: Chuyen sang form quen mat khau', async ({ page }) => {
        await page.locator('text=Quên mật khẩu?').first().click();
        await expect(page.locator('text=ĐẶT LẠI MẬT KHẨU').first()).toBeVisible();
    ...
    test('TC09: Quay lai form dang nhap tu form dang ky', async ({ page }) => {
        await page.locator('text=Đăng ký mới').first().click();
        await expect(page.locator('text=ĐĂNG KÝ').first()).toBeVisible();
        await page.locator('text=Đã có tài khoản? Đăng nhập').first().click();
        await expect(page.getByRole('heading', { name: 'ĐĂNG NHẬP' })).toBeVisible();
    });
    ...
    test('TC10: Quay lai form dang nhap tu form quen mat khau', async ({ page }) => {
        await page.locator('text=Quên mật khẩu?').first().click();
        await expect(page.locator('text=ĐẶT LẠI MẬT KHẨU').first()).toBeVisible();
        await page.locator('text=Quay lại đăng nhập').first().click();
        await expect(page.getByRole('heading', { name: 'ĐĂNG NHẬP' })).toBeVisible();
    });
