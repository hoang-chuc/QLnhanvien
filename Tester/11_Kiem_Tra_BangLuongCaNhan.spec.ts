...
test.describe('TEST 11: BANG LUONG CA NHAN (Nhan vien)', () => {
    ...
    test('TC01: Hien thi trang bang luong ca nhan', async ({ page }) => {
        await expect(page.locator('text=Thu nhập cá nhân')).toBeVisible();
        await expect(page.locator('text=Lịch sử nhận lương')).toBeVisible();
    });
    ...
    test('TC04: Hien thi ghi chu cong thuc tinh luong', async ({ page }) => {
        await expect(page.locator('text=Tổng lương thực lĩnh')).toBeVisible();
    });
